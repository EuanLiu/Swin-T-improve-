...
from .swin_model import swin_small_patch4_window7_224
...
class Network(nn.Module):
    ...
    def __init__(self, channel=32, imagenet_pretrained=True):
        ...
        self.swin_transfomer = swin_small_patch4_window7_224(num_classes=2)

    def forward(self, x):
        # Feature Extraction
        # TODO change swin-transformer backbone
        # x bs 3 352 352

        # SwinTransformer.layers
        ...
        x1, H1, W1 = self.swin_transfomer.patch_embed(x)
        x1 = self.swin_transfomer.pos_drop(x1)

        x2, H2, W2 = self.swin_transfomer.layers[0](x1, H1, W1)
        x3, H3, W3 = self.swin_transfomer.layers[1](x2, H2, W2)
        x4, H4, W4 = self.swin_transfomer.layers[2](x3, H3, W3)
        # x4, H4, W4 = self.swin_transfomer.layers[3](x3, H3, W3)

        # x1 = self.swin_transfomer.norm(x1)
        # x2 = self.swin_transfomer.norm(x2)
        # x3 = self.swin_transfomer.norm(x3)
        # x4 = self.swin_transfomer.norm(x4)

        x1_channel = x1.shape[-1]
        x2_channel = x2.shape[-1]
        x3_channel = x3.shape[-1]
        x4_channel = x4.shape[-1]

        x1 = x1.transpose(1, 2).reshape(-1, x1_channel, H1, W1)
        x2 = x2.transpose(1, 2).reshape(-1, x2_channel, H2, W2)
        x3 = x3.transpose(1, 2).reshape(-1, x3_channel, H3, W3)
        x4 = x4.transpose(1, 2).reshape(-1, x4_channel, H4, W4)
