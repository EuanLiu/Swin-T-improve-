...
for d = 1:datasetNum
    ...
    for m = 1:modelNum
        ...
        fprintf(fileID, '(Dataset:%s; Model:%s) Smeasure	wFmeasure	MAE	adpEm   meanEm  maxEm   adpFm   meanFm  maxFm\n',dataset,model); 
        
        fprintf(fileID, '%.3f   %.3f    %.3f    %.3f    %.3f    %.3f    %.3f    %.3f    %.3f',Sm, wFm, mae, adpEm, meanEm, maxEm, adpFm, meanFm, maxFm); 
